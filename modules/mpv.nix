  ...
  flake.wrappers.mpv =
    ...
    }:
    {
      ...
      script =
        ...
        with pkgs.werapi;
        {
          anacreon-mpv-script.path = anacreon-mpv-script;
          autosubsync-mpv.path = autosubsync-mpv;
          mpris.path = mpris;
          mpv-http-mitmytproxy.path = mpv-http-mitmytproxy;
          mpv-image-viewer-detect-image.path = mpv-image-viewer.detect-image;
          mpv-image-viewer-equalizer.path = mpv-image-viewer.equalizer;
          mpv-image-viewer-freeze-window.path = mpv-image-viewer.freeze-window;
          mpv-image-viewer-image-positioning.path = mpv-image-viewer.image-positioning;
          mpv-image-viewer-minimap.path = mpv-image-viewer.minimap;
          mpv-image-viewer-status-line.path = mpv-image-viewer.status-line;
          mpv-websocket-script.path = mpv-websocket-script;
          mpv-youtube-srv3-subs.path = mpv-youtube-srv3-subs;
          progressbar.path = progressbar;
          sponsorblock.path = sponsorblock;
          thumbfast.path = thumbfast;
          webtorrent-mpv-hook.path = webtorrent-mpv-hook;
        };
      ...
      "mpv.input".content = ''
        ...
        ctrl+i {image-viewer} disable-section image-viewer; script-message status-line-disable; set loop-file no; set video-zoom 0; no-osd set panscan 0; no-osd set video-pan-x 0; no-osd set video-pan-y 0; no-osd set video-align-x 0; no-osd set video-align-y 0 # needed as some files get incorrectly detected as images
      ...
      '';
      configDir = {
        "script-opts/thumbfast.conf".content = /* ini */ ''
          network=yes
        '';
        "script-opts/torque-progressbar/main.conf".content = /* ini */ ''
        ...
        '';
        "script-opts/detect_image.conf".content = /* ini */ ''
        ...
        '';
        "script-opts/status_line.conf".content = /* ini */ ''
          enabled=no
        '';
        "script-opts/animecards.conf".content = /* ini */ ''
        ...
        '';
        "script-opts/minimap.conf".content = /* ini */ ''
          enabled=no
        '';
        "script-opts/webtorrent.conf".content = /* ini */ ''
  ...
  flake.modules.${moduleName}.nixos =
    ...
    in
    {
      ...
      config = lib.mkIf cfg.enable {
        environment.systemPackages = [ config.wrappers.mpv.wrapper ];
      };
    };
}
